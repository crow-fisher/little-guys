import { loadGD, UI_STARMAP_ZOOM } from "../../ui/UIData.js";
...
import { adjustBoundsToIncludePoint } from "./starHandlerUtil.js";


export class StarHandler {
    constructor() {
        this.dataCatalogs = [HipparcosCatalog, StellariumCatalog];
        this.sectors = null;
        this.subDivs = 1;
    ...
    sectorIndexToCartesian(bX, bY, bZ) {
        return [
            this.bounds[0] + (bX / this.subDivs),
            this.bounds[1] + (bY / this.subDivs),
            this.bounds[2] + (bZ / this.subDivs)
        ]
    }

    rebuildSectors() {
        this.subDivs = loadGD(UI_SH_SUBDIVISION_SIZE);
        this.sectors = new Array(this.subDivs);

        for (let i = 0; i < this.subDivs; i++) {
            this.sectors[i] = new Array(this.subDivs);
            for (let j = 0; j < this.subDivs; j++) {
                this.sectors[i][j] = new Array(this.subDivs);
                for (let k = 0; i < this.subDivs; k++) {
                    this.sectors[i][j][k] = new StarSector(this.sectorIndexToCartesian(i, j, k));
                }
            }
        }
    }
    ...
    loadData() {
        ...
        this.dataCatalogs.forEach((Catalog) => {
            let curCatalog = new Catalog(this.loadedStars.push, this.loadedConstellations.push);
