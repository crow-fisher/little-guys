...
import { resetStarStyle } from "./IBODUIFunctionMaps.js";

export function IBODModeFuncSelect(window, container, sizeX, sizeY) {
    ...
    () => getIBODComponent().plotStarScatter.sValues.filter((star) => star != null && star.graphVisible).forEach((star) => star.selected = true), 
    "select all", () => getActiveClimate().getUIColorInactiveCustom(0.55)));

        buttonRow.addElement(new Button(window, sizeX / 3, getBaseUISize() * 3, UI_CENTER, 
    () => getIBODComponent().plotStarScatter.sValues.filter((star) => star != null && star.graphVisible).forEach((star) => star.selected = false), 
    "deselect all", () => getActiveClimate().getUIColorInactiveCustom(0.55)));
        buttonRow.addElement(new Button(window, sizeX / 3, getBaseUISize() * 3, UI_CENTER, 
    () => getIBODComponent().plotStarScatter.sValues.filter((star) => star != null && star.graphVisible).forEach((star) => star.selected = !star.selected), 
